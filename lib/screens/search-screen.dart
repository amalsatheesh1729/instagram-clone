...
class _SearchScreenState extends State<SearchScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body:ishowusers? FutureBuilder(
          future: FirebaseFirestore.instance.collection('users').where('username',isGreaterThanOrEqualTo:searcheditcontroller.text).get(),
          builder:(BuildContext context,AsyncSnapshot<QuerySnapshot<Map<String, dynamic>>> snap)
          {
            ...
            return ListView.builder(
                ...
                return InkWell(
                  onTap: (){
                    Navigator.push(context, MaterialPageRoute(builder: (context)=>ProfileScreen(uid:snap.data!.docs[i]['uid'] )));
                  },
                  child: ListTile(
                    leading: CircleAvatar(
                      backgroundImage: NetworkImage(snap.data!.docs[i]['file']),
                    ),
          ...
          builder: (BuildContext context,AsyncSnapshot<QuerySnapshot<Map<String, dynamic>>> snap)
      {
       ...
       final childarr= snap.data!.docs.map((e) => StaggeredGridTile.count(
